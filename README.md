...
Toolkit de conversion de documents (doc/docx -> pdf) autonome sur Windows x86_64, macOS arm64,
et generation de polices (allfontsgen) sur Linux x86_64.
...
Dezipper `convert-windows-x64.zip`, il ne contient qu'un seul fichier : `Abx2t.exe`.

```powershell
.\Abx2t.exe "rapport.docx" "rapport.pdf"
```

Au tout premier lancement, `Abx2t.exe` s'auto-installe (extraction des composants dans
`resources\`, generation des polices systeme dans `allfonts\`) : aucune etape manuelle requise.
